...
public:
    ...
    void moveFront(GameObj3D* obj, float num, bool called);
    void moveRight(GameObj3D* obj, float num, bool called);
    void moveUp(GameObj3D* obj, float num, bool called);
...
void GameObj3D::moveFront(GameObj3D* obj, float num, bool called = false) {
    const glm::vec3 newPos = front() * num + position();
    translate(newPos.x, newPos.y, newPos.z);

    if (isCollid && !called && CollidesWithSth(*obj)) {
        moveFront(obj, -num, true);
    }
}

void GameObj3D::moveRight(GameObj3D* obj, float num, bool called = false) {
    const glm::vec3 newPos = right() * num + position();
    translate(newPos.x, newPos.y, newPos.z);
    
    if(isCollid && !called && CollidesWithSth(*obj)){
        moveRight(obj, -num, true);
    }
}

void GameObj3D::moveUp(GameObj3D* obj, float num, bool called = false) {
    /*
    if (obj->id == 0 && num > 0) 
    {
        landed = false;
...
void DeleteFromScene(vector<GameObj3D*>& scene, int id)
{
    int index = -1;
    for (int i = 0; i < scene.size(); i++)
    {
